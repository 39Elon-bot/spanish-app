/* ¡Escucha! — 聞き流しスペイン語(単語2000+フレーズ) */
"use strict";

/* ══════════════ データ展開 ══════════════ */
const WORD_SECTIONS = [].concat(WORD_PART_1, WORD_PART_2, WORD_PART_3, WORD_PART_4, WORD_PART_5);

function flatten(sections) {
  const out = [];
  sections.forEach((sec) => {
    sec.items.forEach((it) => out.push({ es: it.es, en: it.en, ja: it.ja, cat: sec.cat, icon: sec.icon }));
  });
  return out;
}

const DECKS = {
  words:   { label: "単語帳",   icon: "🎧", items: flatten(WORD_SECTIONS),   cycleSize: 120 },
  phrases: { label: "フレーズ", icon: "💬", items: flatten(PHRASE_SECTIONS), cycleSize: 96 },
};
...
/* ══════════════ 状態管理 ══════════════ */
const LS_KEY = "escucha-v1";
const defaultState = () => ({
  settings: { rate: 0.9, gap: 0.5, itemGap: 1.2, esTwice: false, esVoice: null },
  repeatMode: { words: "cycle", phrases: "cycle" },   // cycle | continue | all
  resume: { words: null, phrases: null },             // 最後に再生したアイテムの通し番号
  cycleMax: { words: {}, phrases: {} },               // サイクル内で聞いた最大数(進捗表示用)
});
...
function updateMediaSession(it) {
  ...
  try {
    navigator.mediaSession.metadata = new MediaMetadata({
      title: it.es,
      artist: `${it.en} / ${it.ja}`,
      album: `¡Escucha! ${DECKS[P.deck].label} サイクル${c}`,
...
let currentView = "words"; // words | phrases | settings | player
...
function renderDeck(deck) {
  ...
  $screen.appendChild(el(`
    <div class="hero">
      <h1>${d.icon} ${deck === "words" ? "聞き流し単語帳" : "聞き流しフレーズ"}</h1>
      <p>${deck === "words"
        ? `日常生活の頻出 ${d.items.length}語 を es → en → ja の順で自動再生。1サイクル約15分、ランニングのお供にどうぞ。`
        : `ネイティブがよく使う短いフレーズ ${d.items.length}本。1サイクル約15分で聞き流し。`}</p>
    ...
    const btn = el(`
      <button class="resume-card">
        ...
        <span>
          ...
          <div class="r-sub">サイクル${c + 1} の ${pos}/${size} — ${esc(d.items[r].es)}</div>
        </span>
      </button>
    `);
  ...
  for (let c = 0; c < n; c++) {
    ...
    const card = el(`
      <button class="cycle-card ${done ? "done" : ""} ${c === curCycle && r != null ? "current" : ""}">
        <span class="c-num">${c + 1}</span>
        <span class="c-main">
          <div class="c-title">サイクル${c + 1} <small style="color:var(--sub);font-weight:400">(${size}${deck === "words" ? "語" : "本"} · 約15分)</small></div>
...
function renderPlayer() {
  ...
  $screen.appendChild(el(`
    <div class="info-card" style="margin-top:16px">
      ...
      もし音が止まる端末は <b>🌙黒画面モード</b> — 画面を真っ黒にしてスリープを防ぎ、確実に再生し続けます。ポケットに入れてランニングへ!</p>
    </div>
  `));
...
function updateNowPlayingUI(it) {
  ...
  if (!$(".now-card")) return;
  $(".player-cycle").textContent = `${DECKS[deck].icon} サイクル${c + 1}/${cycleCount(deck)}`;
...
function renderSettings() {
  ...
  const itemGapRow = el(`
    <div class="setting-row">
      <label>⏭ 次の単語までの間隔 <span id="igap-val">${s.itemGap.toFixed(1)}秒</span></label>
  ...
  const wp = deckProgress("words"), pp = deckProgress("phrases");
  $screen.appendChild(el(`
    <div class="info-card">
      <h3>📊 進捗</h3>
      <p>単語帳: ${wp.heard} / ${wp.total} 語<br>
      フレーズ: ${pp.heard} / ${pp.total} 本</p>
    </div>
  `));
  ...
  $screen.appendChild(el(`
    <div class="info-card">
      <h3>🏃 聞き流しのコツ</h3>
      <p>・再生開始後にロックしても、ロック画面の ⏯ ボタンで操作できます<br>
      ・端末によってはロック中に読み上げが止まることがあります。その場合はプレイヤーの <b>🌙黒画面モード</b> を使うと確実です(画面は真っ黒・スリープしません)<br>
      ・消音スイッチON(マナーモード)だと音が出ない端末があります<br>
      ・進捗は自動保存。アプリを閉じても「続きから再生」で再開できます</p>
    </div>
  `));
  ...
  $screen.appendChild(el(`
    <div class="info-card">
      <h3>🇪🇸 学習アプリ</h3>
      <p>ユニット学習・クイズ・スピーキング練習は <a href="../">¡Hablemos!</a> でどうぞ。</p>
    </div>
  `));
  ...
  resetCard.querySelector("button").addEventListener("click", () => {
    if (confirm("再生位置と進捗をすべてリセットしますか?(設定は残ります)")) {
      state.resume = { words: null, phrases: null };
      state.cycleMax = { words: {}, phrases: {} };
      saveState();
      renderSettings();
    }
  });
